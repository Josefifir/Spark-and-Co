...
export async function incrementDiscountCodeUsage(code, discountAppliedCents = 0) {
  try {
    await dbConnect();
    const doc = await DiscountCode.findOne({ code: code.toUpperCase() });
    if (!doc) return;

    doc.usageCount += 1;

    // For gift cards: deduct the amount actually applied
    if (doc.discountType === "gift_card" && discountAppliedCents > 0) {
      const current = doc.remainingBalanceCents ?? doc.discountValue;
      doc.remainingBalanceCents = Math.max(0, current - discountAppliedCents);
      if (doc.remainingBalanceCents === 0) doc.isActive = false;
    }
