...
ProductSchema.index({ category: 1, featured: -1, createdAt: -1 }); // Category filtering with featured first
ProductSchema.index({ isActive: 1, featured: -1, createdAt: -1 }); // Active products with featured first
ProductSchema.index({ slug: 1 }, { unique: true }); // Fast slug lookup (already in schema but explicit)
ProductSchema.index({ sku: 1 }, { unique: true }); // Fast SKU lookup (already in schema but explicit)
