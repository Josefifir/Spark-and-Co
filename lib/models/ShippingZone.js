...
// Allows the checkout to resolve a rate by its subdocument _id without a full collection scan
shippingZoneSchema.index({ "rates._id": 1, enabled: 1 });
...
shippingZoneSchema.methods.calculateRate = function(orderDetails) {
  const { subtotalCents, weightGrams } = orderDetails;
