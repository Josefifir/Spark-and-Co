...
export async function createTotpPending(adminUser) {
  ...
  cookieStore.set(TOTP_PENDING_COOKIE, id, {
    ...
    secure:   process.env.NODE_ENV === "production",
    sameSite: "lax",
    path:     "/",        // must cover both /admin/login/totp and /api/admin/totp-verify
...
export async function verifyCsrf(request) {
  ...
  if (!sessionId) {
    console.warn("[verifyCsrf] No session cookie found");
    return false;
  }
  const header = request.headers.get("x-csrf-token");
  if (!header) {
    console.warn("[verifyCsrf] No x-csrf-token header in request");
    return false;
  }
  const expected = signCsrf(sessionId);
  const headerBuf   = Buffer.from(header,   "hex");
  const expectedBuf = Buffer.from(expected, "hex");
  if (headerBuf.length === 0 || headerBuf.length !== expectedBuf.length) {
    console.warn("[verifyCsrf] Buffer length mismatch — header len:", headerBuf.length, "expected len:", expectedBuf.length);
    return false;
  }
  const { timingSafeEqual } = await import("crypto");
  const ok = timingSafeEqual(headerBuf, expectedBuf);
  if (!ok) console.warn("[verifyCsrf] Token mismatch — header:", header.slice(0, 8), "… expected:", expected.slice(0, 8), "…");
  return ok;
}
