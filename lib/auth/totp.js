...
export function generateTotpSecret(accountEmail, issuer = "Strike & Co. Admin") {
  // new OTPAuth.Secret() generates a random 20-byte (160-bit) secret by default
  ...
  const encrypted = encryptTotpSecret(base32);
  return { secret: base32, encrypted, otpauthUrl: totp.toString() };
}
