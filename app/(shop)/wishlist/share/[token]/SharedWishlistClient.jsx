...
export default function SharedWishlistClient({ wishlist, expiresAt }) {
  const { addItem } = useCart();
  const { formatPrice } = useCurrency();

  const expDate = expiresAt ? new Date(expiresAt).toLocaleDateString() : null;

  return (
    <div className="max-w-3xl mx-auto px-6 py-10">
      <div className="flex items-center gap-3 mb-2">
        <Heart className="w-5 h-5 text-flame" />
        <h1 className="font-display text-2xl font-bold text-paper">Shared Wishlist</h1>
        <span className="text-steel text-sm">({wishlist.length})</span>
      </div>
      {expDate && (
        <p className="text-xs text-steel mb-8">This wishlist link is valid until {expDate}.</p>
      )}
      ...
      {wishlist.length === 0 ? (
        <div className="border border-dashed border-hairline rounded-sm p-16 text-center">
          ...
          <p className="text-paper-dim mb-4">This wishlist appears to be empty.</p>
              ...
              <div key={product._id} className="flex items-center gap-4 border border-hairline rounded-sm p-4">
                ...
                {product.stock > 0 && (
                  <button
                    ...
                    className="h-9 w-9 flex items-center justify-center rounded-sm border border-hairline hover:border-flame hover:text-flame text-steel transition-colors shrink-0"
      ...
      <div className="mt-10 text-center">
        <Link href="/products" className="text-flame hover:text-flame-bright text-sm underline">
          Shop all products →
        </Link>
      </div>
    </div>
